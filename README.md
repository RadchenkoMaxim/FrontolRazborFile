...
Текущая версия: **1.4.1**. Разработчик: [Maximum IT](https://maximumit25.ru/nfr).
...
- использует спокойную цветовую систему и единообразные векторные иконки без лишних рамок;
- поддерживает мультивыбор видов маркировки, последние открытые файлы и переход к физической строке по `Ctrl+G`;
- показывает сводную статистику файла и распределение товаров по видам маркировки;
- отображает пустые значения как «не передано» и показывает полные значения обрезанных ячеек во всплывающих подсказках.
- подписывает назначение каждой команды в фильтре и заголовках групп, а подробное действие показывает при наведении.
...
Готовая версия находится в `artifacts/FrontolFileAnalyzer-1.4.1-win-x64-self-contained`. Запустите единственный файл `FrontolFileAnalyzer.exe`, нажмите «Открыть» или перетащите файл обмена в окно.
...
dotnet publish .\src\FrontolFileAnalyzer\FrontolFileAnalyzer.csproj -c Release -r win-x64 --self-contained true --no-restore -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:EnableCompressionInSingleFile=true -o .\artifacts\FrontolFileAnalyzer-1.4.1-win-x64-self-contained
```
